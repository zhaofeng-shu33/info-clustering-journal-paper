...
our improved algorithm, which is one order of magnitude faster than previous. Besides clustering analysis,our method can be adopted and
...
Many problems in graph model are NP-Hard and only allows approximate solutions. Quite surprisingly, our proposed graph-based clustering method is polynomial-time efficient solvable exactly.
...
by \citet{pin}. In this article, we will propose a new improvement to compute PSP, called PSPI. We will deduce the time complexity of PSPI under some mind assumptions. And we will show that empirically our improvement runs almost an order of magnitude faster than other methods.

As far as we know, the mathematical expression of info-clustering on graph model is similar to the minimum average cost \citep{mac} and network information flow \citep{pin}. Therefore, the algorithmic implementations, i.e., to compute PSP, are the same within these methods. We call them PSP-based methods. However, Different methods
are based on different assumptions and the theoretical discussion differs between these methods. Besides, in this article, some theoretical
properties are analyzed specifically for graph model, which is absent from prior work. These properties inspires us to extend our method to many different data-mining problems.
...
we will not only consider data clustering problems, in which our proposed method can be directly applied. We will also consider link prediction, outlier detection and community discovery problems, in which we will propose new promising
...
The paper is organized as follows: In Section \ref{sec:bk} we introduce the background knowledge of the theoretical Info-Clustering, its relationship with PSP structure and theory of local geometry information. In Section \ref{sec:GBIC} our formulation of GBIC and important properties of our model are given. In Section \ref{sec:alg}, we propose an improved algorithm to compute the PSP structure of graph. Then in Section \ref{sec:es} we conduct empirical study of GBIC on data clustering and extend the method to other fields including link prediction, outlier detection and community discovery. Finally we give the conclusion in Section \ref{sec:conc}.
...
\end{equation}
To simplify the notation, the term $\sum_{x\in \mathcal{X}, y\in \mathcal{Y}} B^2(x,y)$ can be regarded as the square of Frobenius norm $\norm{B}_F^2$ for matrix $B$. Equation \eqref{eq:Ixy} gives the dominant term
of the mutual information when the random variables are weakly dependent.

\subsection{Theoretical Info-Clustering}
There are many ways to extend mutual information for multiple random variables. \cite{ic2016} gives one extension based on partition of set $V$. We use $\Pi'(V)$ to represent all partitions of $V$ except for $\{V\}$. The multivariate mutual information (MMI) of $\{Z_i | i \in V\}$ is defined as:
...
a special kind of submodular function minimization problem is established \citep{mac}. All threshold values and clusters are the same as those of
...
And $C_{\lambda_i} =\{C | C\in \P_i, \abs{C} > 1\}$.
...
In Algorithm \ref{alg:psp}, a function \texttt{DT} is used to get the minimal value of Equation \eqref{eq:hPLambda}.
One of implementation of  \texttt{DT} is described by \cite{mac} and we omit it here.
...
Theorem \ref{thm:DPX} is a statement about random variables. Given a $K-$ cluster dataset with $n$ samples, there are $K$ random variables. 
If we consider a total partition $\P=\{ \{X_1\}, \dots, \{X_K\}\}$, to compute Equation \eqref{eq:PXV} we only need to compute the pairwise B matrix related
quantity. Assume Cluster i $X_i$ with alphabet size $\abs{\mathcal{X}_i}$
such that $\sum_{i=1}^K \abs{\mathcal{X}_i} = n$,
...
\end{equation}
Therefore from Equation \eqref{eq:Ixy}, we have $\norm{B_{ij}}_F^2 = \sum_{x_i \in \mathcal{X}_i, x_j \in \mathcal{X}_j} \phi^2_{X_iX_j}(x_i, x_j)$ and Equation \eqref{eq:PXV} can be computed for the total partition.

For other non-trivial partitions of $X_1, \dots, X_k$, the same procedure can be carried and we have the following expression in terms of data.
\begin{equation}\label{eq:PXV_Data}
D(P_{X_V} || \prod_{C\in \P} P_{X_{C}}) = {1 \over 2}\sum_{\substack{(i,j) \not\in C\\ C\in \P}} \sum_{x_i \in \mathcal{X}_i, x_j \in \mathcal{X}_j}  \phi_{X_iX_j}(x_i, x_j) + o(\epsilon^2)
\end{equation}
We can treat the term $\phi^2_{X_iX_j}(x_i, x_j)$ as the capacity weight for a graph, with each data sample as a vertex.
We then expand each $X_i$ to its alphabet and treat the partition $\P$ as partition of the graph vertices. Also we omit the high order terms $o(\epsilon^2)$ and write
$w_{ij} = \phi^2_{X_{d(x_i)}X_{d(x_j)}}(x_i, x_j)$ where $d(x_i)$ maps the vertex to its underlining random variable index, $1\leq i,j \leq \abs{V}$.
...
$G(V, E)$ where the edge weight $w$ is non-negative and represents pairwise similarity of nodes. For a subset $C$ of $V$, $f(\cdot)$ is graph cut function, that is $f(C) = \sum_{i\not\in C, j\in C, (i,j) \in E} w_{ij}$. Combing Equation \ref{eq:IZV, eq:PXV_Data} we have
...
\end{align}
\end{definition}
The quantity $I(Z_V)$ in equation \eqref{eq:ms} is called MMI of $G$. We can also compute MMI for subgraph $G'(V',E')$ of $G$, denoted by $I(Z_{V'})$. When $V'=\{i,j\}$, we have $I(Z_{V'})=w_{ij}$ from above definition.

To reduce the general model to GBIC, we also assume that each random variable $X_i$ only takes discrete values from $\mathcal{X}_i$. This is all observed samples belong to Cluster i. To apply GBIC on empirical experiment, $w_{ij}$ should be chosen property. From its definition, $w_{ij} \geq 0$. When $w_{ij} = 0$ we say there is no edge between
node $i$ and node $j$. The choice of $w_{ij}$ is not unique. From engineering prospective, the rbf kernel $w_{ij} = \exp(-\gamma \norm{x_i - x_j}^2)$ is a not bad choice
where $\gamma$ is a positive parameter to be tuned.

To perform clustering tasks on our GBIC model, it is the same procedure as the general infor-clustering model. That is, we proceed in a divisive way as Equation \eqref{eq:CZV}. Finally, we can get the cluster tree $\mathcal{T}$. A hierarchical tree $\mathcal{T}$ of $G$ is a rooted tree. Each node in $\mathcal{T}$ is represented by a subset $S \subset V$. Its root node is $V$ and its leaf node is $\{j\}$. Each non-leaf node of $\mathcal{T}$ is called a cluster of graph $G$.
...
Based on our assumption, multiple tree leaf node is assumed to join a stem node $X_i$ or one leaf node is a random variable $X_j$. In the later case, we say $X_j$ is an outlier.
