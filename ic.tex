...
To apply our GBIC method to this scanerio, we consider the case when we are required to detect outliers in a multi-class dataset and then we use critical values of $C_{\gamma}(Z_V)$. Suppose $\lambda_k$ is the largest threshold value and there are $K$ clusters each with 
equal size $N$ and the total number of outliers $M$ is less than $N$,
we should choose the critical value $\gamma_{k+1-K}$ as the threshold value.

In reality it is often the case that we do not know the value of $K$.
An empirical and reasonable way to deal with this situation is trying from $K=1,2, \dots$ and use the smallest suitable one.
The selection rule for $\mathcal{P}_{-k}$ can be summarized as follows:
...
We use $n$ to represent the total number of data. Since we assume $k$ is small, $M \approx |\mathcal{P}_{-k}|$.
And the condition $ n > (k+1) |\mathcal{P}_{-k}|$ is equivalent
to $N>M$ since $n=kN + M$.

We do three simulation to verify our empirical method. The first one is that we put $(0,0)$ to the Four Gaussian Blob dataset;
