...
To simplify the notation, let $w_{ij} = \phi^2_{Z_{d(z_i)}Z_{d(z_j)}}(z_i, z_j)$ \footnote{When $d(z_i) = d(z_j)$, we can still formally define $w_{ij}$ to be some value larger than $\phi^2_{Z_i Z_j}(z_i, z_j)$.} where $d(z_i)$ maps the vertex to its underlining random variable index, $1\leq i,j \leq \abs{V}$.
