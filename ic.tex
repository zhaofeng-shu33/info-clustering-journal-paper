...
%             \emph{Present address:} of F. Author  %  if needed
           \and
           Yang Li, Fei Ma, Shao-Lun Huang and Lin Zhang \at
...
It has wide applications in many areas, such as epidemic
sources tracking \citep{epidemic2020} and natural disasters modeling \citep{earthquake2016}.
...
\begin{equation}
P \in N^{(\epsilon)}(P_0) \iff \sum_{x \in \mathcal{X}} \frac{(P(x) - P_0(x))^2}{P_0(x)} \leq \epsilon^2
\end{equation}
...
$V$ be the index set of random variables,
$Z_V=\{Z_i | i \in V\}$ and we use $\Pi'(V)$ to represent all partitions of $V$ except for $\{V\}$. The multivariate mutual information (MMI) of random variable set $Z_V$ is defined as:
\begin{align}
...
$Z_1, \dots, Z_n (n\geq 3)$ are weakly dependent if there exists a random variable $U$ such that
...
and $\mathrm{Cov}(Z|U)$ is a diagonal matrix. Then by Definition \ref{def:general} we say $Z_1, \dots, Z_n$ are weakly dependent.
...
\hline
Gaussian-blobs & float & $|E|=O(|V|^2)$\\
Two-level graph & int & $|E|=O(|V|^{3/2})$\\
\hline
\end{tabular}
