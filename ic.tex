...
We compare GBIC with agglomerative hierarhical clustering with average distance and Bayesian Rose tree \citep{blundell2011discovering}. The tree similarity metric is Robinson-Foulds distance, which is the number of steps to transform one tree to another (A step is either adding or removing an edge). We vary the samples of data and the results are shown in Figure \ref{fig:shc}. As can be seen, our GBIC method achieves the highest 
