...
by using parametric maximal flow techniques \citep{kolmogorov}. In this article, to compute PSP, we will propose a new algorithm, which is called PSP Improved (PSPI). We will deduce the time complexity of PSPI under some mild assumptions in Section \ref{sec:alg}. And we will also empirically show that our improvement runs almost an order of magnitude faster than other methods.
