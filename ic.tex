...
Theorem \ref{thm:DPX} is a statement about random variables. Now we expand it to data samples. Given a $K-$cluster dataset with $n$ samples, each cluster is a random variable. 
...
With a little abuse of terminology and notation, we use $_{\P}(Z_V)$ to represent the second order coefficient of MMI and call it the MMI of graph-based mutual clustering (GBIC). 
...
\end{figure}
Consider 10 data samples and their feature vectors are on Euclidean plane, as shown in Figure \ref{fig:ta}. The underlining graph with 10 nodes is weighted  by $w_{ij} = \exp(-\norm{x_i - x_j}^2)$. Let $X_1, X_2, X_3, X_4$ represents the square, pentagon, circle and triangle samples respectively. We can write the clustering solution \footnotetext{computed by Algorithms introduced in the next Section} as:
\begin{equation*}
\P = 
\begin{cases}
...
\end{cases}
\end{equation*}
When $\lambda > 0.98$, some clusters are split further to data samples and we omit the partition of data samples. Although the clustering tree does not distinguish $\{\{X_1\},\{X_2\},\{X_3,X_4\}\}$ from $\{\{X_1,X_2\},\{X_3\}, \{X_4\}\}$, the partition $\{\{X_1,X_2\},\{X_3\}, \{X_4\}\}$ does not exist.
\end{example}
