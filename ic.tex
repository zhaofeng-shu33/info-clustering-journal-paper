...
In Algorithm \ref{alg:psp}, a function DT is used to get the miminal value, which is proposed by \cite{mac} and has advantages over the original fusion set method to compute $h(\lambda)$ for a given $\lambda$.

For general submodular function $f[\mathcal{P}]$ there has been some practical algorithms to optimize it \citep{chakrabarty2014provable} but it is infeasible to apply such kind of algorithms when floating point
.
