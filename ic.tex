...
clusterpath based on convex optimization \cite{hocking2011clusterpath}, graph partition based on Dasgupta's cost \cite{dasgupta2016cost}, Gradient-based Hierarchical Clustering based on hyperbolic gemetry \cite{hyperbolic} etc. These methods have both advantages and disadvantages. In practice researchers need to choose the proper method according to the specific problem. For example, Bayesian hierarchical clustering can produce non-binary hierarchical tree under a given probabilistic model [citation needed] and have XXX advantages. But Bayesian model has many hyper-parameters to tune and the clustering results between different runs vary greatly because of its intrinsic randomness.
